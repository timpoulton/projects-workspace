...
# Configuration
PORT = 5001  # STANDARDIZED: Category E (External APIs) - Per PORT-TRACKER.md
...
class UpworkProposalServer(http.server.BaseHTTPRequestHandler):
        ...
        if self.path == '/webhook/rss-jobs':
            ...
            try:
                ...
                for job in jobs:
                    ...
                    # Only save if proposal was generated (not rejected)
    ...
    def process_job(self, job):
        ...
        # Generate consistent timestamp for both URL and filename
        timestamp = int(time.time() * 1000)
        
        # Analyze job with enhanced scoring
        job_analysis = self.analyze_job(job_title, job_description)
        
        # Check if job was rejected
        if job_analysis.get('rejected', False):
            print(f"🚫 SKIPPING REJECTED JOB: {job_title[:50]}... | Reason: {job_analysis.get('reason', 'Unknown')}")
            return None  # Don't process rejected jobs
        
        # Generate proposal with Gemini for accepted jobs only
...
Timothy"""
    ...
    def save_to_queue(self, proposal_data):
        ...
        try:
            ...
            # Run the direct sync script to update the dashboard immediately
            sync_script = os.path.join(os.path.dirname(__file__), 'direct-sync.sh')
            if os.path.exists(sync_script):
                try:
                    subprocess.run([sync_script], check=True, timeout=10)
                    print(f"✅ Dashboard data synced with direct-sync.sh")
                except Exception as e:
                    print(f"⚠️ Warning: Failed to run direct sync script: {e}")
    ...
    def save_proposal_html(self, proposal_data):
        ...
        filename = proposal_data.get('filename') or self.generate_filename(proposal_data['job_title'])
